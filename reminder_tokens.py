import base64
import os
import sqlite3

import pandas as pd
import svgwrite

from shared import ensure_dir

TOKEN_SIZE = 0.75
TOKEN_BUFFER = TOKEN_SIZE + 0.1

REMINDER_DIR = "reminder_tokens"

def build_sheet(db, sheet, tokens, board_width_in, board_height_in, out_path):
    ensure_dir(out_path)
    cols = max(1, int(board_width_in // TOKEN_BUFFER))
    dwg = svgwrite.Drawing(out_path, size=(f"{board_width_in}in", f"{board_height_in}in"), profile='full')
    # embed_fonts(dwg) bring back later

    placed = 0
    for i, (name, char_type, imgPath) in enumerate(tokens):
        # get character imgPath
        # iterate over columns
        col, row = i % cols, i // cols
        x, y = col * TOKEN_BUFFER, row * TOKEN_BUFFER
        if y + TOKEN_BUFFER > board_height_in:
            break
        add_token(dwg, x, y, name, char_type, imgPath)
        placed += 1

    rows_used = -(-placed // cols) if placed else 0
    print(f"Board completed after {placed} tokens ({cols} cols x {rows_used} rows) — {placed} placed, {len(tokens) - placed} left")
    dwg.save()
    return placed

def add_token(dwg, x, y, name, char_type, imgPath):
    token = dwg.svg(insert=(f"{x}in", f"{y}in"), size=(f"{TOKEN_SIZE}in", f"{TOKEN_SIZE}in"))
    dwg.add(token)

    with open(imgPath, "rb") as f:
        b64 = base64.b64encode(f.read()).decode("ascii")
    href = f"data:image/png;base64,{b64}"

    img_size = 0.25
    circle_center = TOKEN_SIZE / 2
    nudge_up = 0.05 #increase this to move image higher up
    insert_x = circle_center - img_size / 2 #horizontally centering the image
    insert_y = circle_center - img_size / 2 - nudge_up #vertically centering the image and then moving it a bit up to avoid text overlap

    img = dwg.image(href, insert=(f"{insert_x}in", f"{insert_y}in"), size=(f"{img_size}in", f"{img_size}in"))
    img.fit(horiz='center', vert='middle', scale='meet')
    token.add(img)

    token.add(dwg.circle(center=("1in", "1in"), r="1in", fill="none", stroke="red", stroke_width="0.001in"))
    token.add(dwg.text(name, insert=("1in", "1.51in"), fill="black", font_size="16pt",
                        font_family="Franklin Gothic Book", text_anchor="middle"))
    token.add(dwg.text(char_type, insert=("1in", "1.6982in"), fill="black", font_size="12pt",
                        font_family="Franklin Gothic Demi Cond", font_weight="bold", text_anchor="middle"))


def main(img_db, spreadsheet = "base_and_carousel.xlsx", **kwargs):
    script = kwargs.get("script_name", "board")
    width = kwargs.get("width", 24)
    height = kwargs.get("height", 12)

    db = sqlite3.connect("reminders.db")
    db.execute("""CREATE TABLE IF NOT EXISTS characters (
                    name            TEXT,
                    token_1         TEXT DEFAULT NULL,
                    token_2         TEXT DEFAULT NULL,
                    token_3         TEXT DEFAULT NULL,
                    token_4         TEXT DEFAULT NULL,
                    token_5         TEXT DEFAULT NULL,
                    token_6         TEXT DEFAULT NULL,
                    token_7         TEXT DEFAULT NULL,
                    svg_made        INTEGER DEFAULT 0,
                    img_filepath    TEXT DEFAULT NULL
                    )
                """)
    db.commit()
    df = pd.read_excel(spreadsheet)
    for row in range(df.shape[0]):
        name = df.at[row, "Name"]
        img = img_db.execute("""
                        SELECT img_filepath 
                        FROM characters
                        WHERE character_name IS ?
                        """, (name,)).fetchall()[0].lower().replace(" ", "").replace("-", "").replace("'", "").replace("(ugmode)", "")
        token_1 = df.at[row, "Token"]
        token_2 = df.at[row, "Token"] #how to handle columns being named the same
        token_3 = df.at[row, "Token"]
        token_4 = df.at[row, "Token"]
        token_5 = df.at[row, "Token"]
        token_6 = df.at[row, "Token"]
        token_7 = df.at[row, "Token"]
        db.execute("""
                    INSERT OR IGNORE INTO characters  
                    (name, token_1, token_2, token_3, token_4, token_5, token_6, token_7, img_filepath) 
                    VALUES (?, ?, ?)
                    """, (name, token_1, token_2, token_3, token_4, token_5, token_6, token_7, img))
        #print(name, type, url)
    db.commit()


    board_count = 1
    done = False
    while not done:
        to_tokenize = db.execute("""
                                SELECT character_name 
                                FROM characters
                                WHERE img_filepath IS NOT NULL
                                """).fetchall()

        if not to_tokenize:
            done = True
            break

        build_sheet(spreadsheet, to_tokenize, width, height, os.path.join(REMINDER_DIR, f"{script}_{board_count}.svg"))
        db.commit()
        board_count += 1
    
    
if __name__ == "__main__":
    main()

    #handle multiple reminder tokens that are same name and character
    #handle when board fills up partway through a characters tokens
